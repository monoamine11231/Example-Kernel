...
#[inline(always)]
pub fn inl_outb(port: u16, value: u8) {
    unsafe {
        asm!("mov dx, {0:x}",
             "mov al, {1}",
             "out dx, al",
...
/// Output a WORD to a 16-bit serial port address
#[inline(always)]
pub fn inl_outw(port: u16, value: u16) {
    unsafe {
        asm!("mov dx, {0:x}",
             "mov ax, {1:x}",
             "out dx, ax",
...
/// Output a DWORD to a 16-bit serial port address
#[inline(always)]
pub fn inl_outd(port: u16, value: u32) {
    unsafe {
        asm!("mov dx, {0:x}",
             "mov eax, {1:e}",
             "out dx, eax",
