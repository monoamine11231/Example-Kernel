...
/*
12 pixels width, 16 pixels height
So 1.5 bytes per row
...
fn get_font_entry(c: char) -> usize {
    return (c as usize - 32) * 24; //Since there are 24 u8 per entry
}
...
pub struct Surface {
    ...
    buffer: [[ColorCode; 20]; 20],
}

impl Surface {
    pub fn from_font(c: char) -> Self {
        let mut new_surface = Surface {
            width: 12,
            height: 16,
            origin: Vec2::<usize>::new(0, 0),
            buffer: [[ColorCode::Black; 20]; 20],
        };
        let font_map_idx = get_font_entry(c);
        qemu_fmt_println(
            "{}",
            format_args!("We got the font index: {}", font_map_idx),
        );
        for i in 0..24 {
            for j in 0..8 {
                ...
                if (FONT_MAPPING[font_map_idx + i] & (128 >> j) > 0) {
                    qemu_println("Writing white!");
                    new_surface.buffer[curr_row][curr_col] = ColorCode::White;
                } else {
                    new_surface.buffer[curr_row][curr_col] = ColorCode::Black;
                }
            }
        }
        return new_surface;
